...
El relleno durante la voxelización utiliza la misma clasificación de vidrio para recorrer el exterior: atraviesa celdas Glass sin vaciarlas ni cambiar sus IDs y conserva el aire conectado a ellas desde fuera. Sólo rellena los huecos que permanecen encerrados por geometría opaca. El comportamiento no depende de la opacidad de la LUT; la ruta RGB conserva el relleno por ocupación. Los volúmenes rellenos previamente requieren reconversión desde la geometría fuente, no sólo reconstrucción de mallas.

La rasterización resuelve las celdas compartidas por opaco y vidrio a favor de la muestra opaca válida, después de aplicar el recorte por alpha. Dentro de cada clase conserva la muestra más cercana y la regla existente de desempate por orden. La prioridad opaca no depende del orden de los objetos y evita que el vidrio abra una conexión de relleno a través de una pared que comparte su celda. No almacena dos superficies por voxel ni cambia la política del reductor; puede reducir detalles transparentes menores que la celda.
...
La asignación en `Semantic Bindings` afecta a todos los voxels que utilizan un slot. `Surface Painter`, disponible desde `Edit` en una fila de LOD, `Edit Surfaces` en una salida independiente o desde `Tools > Voxel Bridge`, modifica ColorID o SurfaceID de las celdas seleccionadas conservando el otro atributo. La pintura de color respeta el perfil cromático vinculado. El [procedimiento de edición](README.md#edición-visual-de-superficies) describe selección, comparación temporal de acabados, guardado, recuperación y reconstrucción.
...
3. Revisar `ColorID + SurfaceID` con `⋯ > Semantic Bindings` y editar LOD0 mediante `VOX` para abrir MagicaVoxel, desde el Inspector del prefab. Utilizar `Rebuild` después de guardar.
4. Seleccionar el prefab de familia y abrir `Create LOD N ▾`. `Duplicate Previous` copia el nivel anterior sin reinterpretar sus slots; `Reduce Previous` prioriza los pares de las caras expuestas, con mayoría por volumen para interiores y desempate estable. `Generate Remaining LODs by Reduction` crea únicamente los niveles que faltan.
5. Utilizar `Edit`, `VOX` y `Rebuild` en la fila de cada LOD; `Semantic Bindings` y `Select Source VOX` están en `⋯`. Guardar en MagicaVoxel no reconstruye automáticamente el prefab.
