...
| Geometría limpia y detalle separado | Rejilla opcional de normales y rugosidad con perfiles de líneas o juntas biseladas, POM cercano y hundimientos estables por SurfaceID; modos fijo y multiescala binaria, anclaje mundial o de familia y validador de su marco local. Variante experimental con escritura conservativa de profundidad, sin modificar mallas ni colisiones. Pendientes: evaluación de AO e intersecciones del relieve, SPOM, validación artística y temporal, colocación estática, integración con Entities Graphics, anuncios sobre quads y mediciones. |
...
## Relieve voxel POM/SPOM — ensayo cercano
...
El asset candidato `Assets/Frostzone/CSPOM` incluye documentación, subgrafos separados de POM y silueta, un modo curvo opcional y un ejemplo plano. Su POM utiliza una textura de alturas y UV convencionales; su recorte utiliza límites de UV. El ensayo de Voxel Bridge emplea una implementación propia sobre el perfil procedural biselado para conservar los canales de IDs y el anclaje de familia, sin dependencia ni copia de código del asset.

`Grid POM` ofrece una comparación cercana desactivada por defecto, con profundidad física limitada y desvanecimiento por distancia, ángulo y primera transición visual. No escribe offset de profundidad ni recorta siluetas. La evaluación de SPOM requiere límites de cara compatibles con chunks; no reutilizar directamente el recorte por UV sobre los IDs. Consultar los controles y límites en [detalle superficial](ART_DIRECTION_AND_DETAIL.md#pom-procedural-cercano--experimental). La compatibilidad de producción y el rendimiento requieren pruebas.
