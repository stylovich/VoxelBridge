...
`Reutilizar prefab de origen` evita voxelizar varias veces instancias equivalentes: las instancias sin overrides del mismo prefab comparten una sola familia `.vox` y el mismo prefab convertido. Los cambios normales de posición, rotación y escala del root no cuentan como edición y se recuperan al colocar el resultado. Las variantes conservan su prefab fuente inmediato y no se mezclan automáticamente con el prefab base.
...
`Colocar resultado en escena` crea una raíz hermana con una instancia voxel por cada hijo convertido y conserva sus transforms, estado activo, layer y flags Static. Si `Desactivar raíz original` está activo, la sustitución solo ocurre cuando todos los hijos directos tienen malla y todos los modelos terminan correctamente; ante cancelaciones, errores, instancias ignoradas o hijos sin malla, la nueva raíz queda desactivada y la original permanece activa. Toda la operación admite Undo. La raíz generada contiene la representación visual, pero no copia scripts, colliders ni otros componentes del padre original, así que el reemplazo automático debe usarse con raíces exclusivamente visuales.
